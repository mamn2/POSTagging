...
def calcEmissionProbs(emissionCounts, uniqueTags, uniqueWords, hapaxTags, numHapaxWords, smoothingParam):
    ...
    for tag in uniqueTags:
        if tag in hapaxTags:
            hapaxProbs[tag] = (hapaxTags[tag] + .0001) / (numHapaxWords + len(uniqueTags) * smoothingParam)
        else:
            hapaxProbs[tag] = .0001 / (numHapaxWords + len(uniqueTags) * smoothingParam)
    ...
    for i in range(1, len(sentence)):
        ...
        for j in range(len(tags)):
            ...
            if tags[j] in emissionProbs and word in emissionProbs[tags[j]]:
                ...
                if len(word) in lenProbs:
                    emissionsProbability = (emissionProbs[tags[j]]['UNSEEN']) + (lenProbs[len(word)][tags[j]])
                else:
